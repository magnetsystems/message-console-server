define(['jquery', 'backbone'], function($, Backbone){
    var View = Backbone.View.extend({
        ...
        events: {
            ...
            "click .pagination li a": "paginate",
            ...
            me.ignoreTimer(function(){
                if(!e){
                    if(sortDefault){
                        ...
                        if(sortDefault.order == 'desc'){
                            th.children('span').addClass('icon-arrow-up');
                        }
                    }else{
                ...
                if(property){
                    if(!me.options.disableControls){
                        if(th.find('span').hasClass('icon-arrow-up')){
                            th.find('span').removeClass().addClass('icon-arrow-down');
                        }else{
                            th.find('span').removeClass().addClass('icon-arrow-up');
                        }
                        var sorts = me.$el.find('.breadcrumb > span');
                        if(!sorts.find('button[p="'+property+'"]').length){
                            sorts.append('<button class="btn" type="button" p="'+property+'">'+th.html()+'<span class="icon-remove"></span></button>');
                        }else{
                            sorts.find('button[p="'+property+'"]').html(th.html()+'<span class="icon-remove"></span>');
                        }
                        me.collectSorts();
                    }
                    me.fetch();
        ...
        collectSorts: function(){
            ...
            me.$el.find('.breadcrumb span button').each(function(){
                me.query.sorts[$(this).attr('p')] = $(this).find('span').hasClass('icon-arrow-up') ? 'asc' : 'desc';
            });
            me.query.nextPage = '';
            me.query.page = '';
        },
