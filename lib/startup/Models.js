...
StartupModels.prototype.modelSetup = function(callback){
    ...
    if(process.env.NODE_ENV != 'test') me.startJobs();
};

StartupModels.prototype.setupConfig = function(callback){
    orm.model('AppConfig').find({where:['1=1']}).success(function(appConfig){
        if(appConfig){
            ...
            orm.model('AppConfig').create({
                skipAdminApproval : false
            }).success(function(appConfig){
                APP_CONFIG = appConfig;
                winston.info('AppConfig: app configuration has been created an applied successfully.');
                callback();
            }).error(function(e){
...
StartupModels.prototype.startJobs = function(){
    Jobs.create('Announcements', ENV_CONFIG.General.syncNewsInterval, function(opts){
    ...
    Jobs.start('Announcements', function(job, hasFailed){
        if(!hasFailed){
