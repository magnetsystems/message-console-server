...
var MMXManager = function(){
    var me = this;
    me.repo = {};
    var quickstartPath = './mmx/apps/quickstart';
    if(!fs.existsSync('./quickstart-android') && fs.existsSync(quickstartPath)){
        fs.renameSync(quickstartPath, './quickstart-android');
    }
...
MMXManager.prototype.initMMXSampleAppGit = function(platform, cb){
    ...
    deleteFolder('./quickstart');
    git.init('quickstart', function(e, repo){
        if(e && e.code == 'ENOENT'){
            winston.info('Git: the folder "quickstart" did not exist, attempting to clone repository.');
            git.clone(ENV_CONFIG.MMX[platform].repo, 'quickstart', function(e, repo){
                if(e){
                    winston.error('Git: error cloning repo: ', e);
                    cb('error-cloning-repo');
                }else{
                    me.repo[platform] = repo;
                    me.repo[platform].checkout(ENV_CONFIG.MMX[platform].branchName, function(e, report){
                        if(e){
                            winston.error('Git: error checking out branch: ', e);
                            cb('error-checking-out-branch');
                        }else{
                            var quickstartPath = './quickstart/apps/quickstart';
                            if(platform == 'android' && fs.existsSync(quickstartPath)){
                                fs.renameSync(quickstartPath, './quickstart-android');
                            }
