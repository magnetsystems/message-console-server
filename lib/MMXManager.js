...
MMXManager.prototype.request = function(path, method, data, cb, params){
    ...
    var req = {
        ...
        headers : {
            ...
            'Content-Length' : reqBody ? Buffer.byteLength(reqBody, 'utf8') : 0,
    ...
    var call = protocol.request(req, function(res){
        ...
        res.on('data', function(chunk){
            ...
            if(!isSuccess(res.statusCode)){
                winston.error('MMXManager: failed to make remote call to '+params.config.host+(req.port ? ':'+req.port : '')+path+': code - '+res.statusCode+' response: ', data, ' request: ', (!params.binary ? reqBody : ' binary'), res.statusCode);
            }
...
MMXManager.prototype.createAppTopic = function(userMagnetId, mmxId, req, cb){
    ...
    me.request('/mmxmgmt/api/v1/topics', 'POST', req.body, function(e, data, code){
        ...
        winston.verbose('MMXManager: successfully created mmx app topic: ' + req.body.topic);
        cb(null, data, code);
    }, {
        contentType : 'application/json',
        port        : ENV_CONFIG.MMX.publicPort,
