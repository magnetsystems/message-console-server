...
MMXManager.prototype.createAppTopic = function(userMagnetId, mmxId, req, cb){
    var me = this;
    if(!req.body || !req.body.topicName) return cb('invalid-body');
...
MMXManager.prototype.storeAPNSCertificateAndPass = function(userMagnetId, mmxId, req, cb){
    var me = this;
    if(!req.query.apnsCertPassword) return cb('invalid-password');
    if(!req.files.filename) return cb('invalid-file');
    var fileName = req.header('x-file-name');
    var form = new FormData();
    form.append('certfile', fs.createReadStream(req.files.filename.path));
    form.append('apnsCertPassword', req.query.apnsCertPassword);
...
MMXManager.prototype.createSamples = function(user, cb){
    var me = this;
    me.createApp(user.email, user.magnetId, {
        name : 'Quickstart'
    }, function(e1, quickstartApp){
        if(!e1){
            me.createAppTopic(user.magnetId, quickstartApp.appId, {
                body : {
                    topicName : 'all_ios'
                },
                headers : {
                    appapikey : quickstartApp.appAPIKey
                }
            }, function(){});
            me.createAppTopic(user.magnetId, quickstartApp.appId, {
                body : {
                    topicName : 'all_android'
                },
                headers : {
                    appapikey : quickstartApp.appAPIKey
                }
            }, function(){});
            me.sendMessage(user.magnetId, quickstartApp.appId, '', {
                body : {
                    'receipt'  : true,
                    'metadata' : {
                        'content-type'     : 'text',
                    ...
                    'deviceId' : quickstartApp.appId+'-amazing_bot-bot-device',
                    'content'  : 'Hello Amazing Bot!'
                },
                headers : {
                    appapikey : quickstartApp.appAPIKey
                }}, function(e, response){ });
            me.sendMessage(user.magnetId, quickstartApp.appId, '', {
                body : {
                    ...
                    'deviceId' : quickstartApp.appId+'-echo_bot-bot-device',
                    'content'  : 'Hello Echo Bot!'
                },
                headers : {
                    appapikey : quickstartApp.appAPIKey
                }}, function(e, response){ });
        ...
        me.createApp(user.email, user.magnetId, {
            name : 'RPSLS'
        }, function(e2, rpslsApp){
            me.createAppTopic(user.magnetId, rpslsApp.appId, {
                body : {
                    topicName : 'availableplayers'
                },
                headers : {
                    appapikey : rpslsApp.appAPIKey
                }
            }, function(){});
            me.createApp(user.email, user.magnetId, {
                name : 'Soapbox'
            }, function(e3, soapboxApp){
                me.createAppTopic(user.magnetId, soapboxApp.appId, {
                    body : {
                        topicName : 'lunch_buddies'
                    },
                    headers : {
                        appapikey : soapboxApp.appAPIKey
                    }
                }, function(){});
                me.createAppTopic(user.magnetId, soapboxApp.appId, {
                    body : {
                        topicName : 'company_announcements'
                    },
                    headers : {
                        appapikey : soapboxApp.appAPIKey
                    }
                }, function(){});
                //if(e1 || e2 || e3) return cb(e1 || e2 || e3);
                cb(null, user, true);
            });
        });
    });
