...
MMXManager.prototype.getStats = function(uid, cb){
    var me = this;
    me.request('/plugins/mmxmgmt/rest/v1/stats/app', 'GET', '', function(e, data){
        if(e) return cb(e);
        cb(null, data);
    });
};
...
MMXManager.prototype.getSample = function(userMagnetId, mmxId, platform, cb){
    ...
    me.request('/plugins/mmxmgmt', 'GET', {
        ...
        zip.addLocalFolder('./tmp/samples/'+platform);
        zip.addFile('tmp/samples/'+platform+'/app/src/main/res/raw/quickstart.properties', new Buffer(tmpl), 'sample data');
...
MMXManager.prototype.initMMXSampleAppGit = function(platform, cb){
    ...
    deleteFolder('./'+GIT_DIR+'/'+platform);
    git.init(GIT_DIR+'/'+platform, function(e, repo){
        if(e && e.code == 'ENOENT'){
            winston.info('Git: the folder "'+GIT_DIR+'/'+platform+'" did not exist, attempting to clone repository.');
            git.clone(ENV_CONFIG.MMX[platform].repo, GIT_DIR+'/'+platform, function(e, repo){
                if(e){
