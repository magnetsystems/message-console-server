...
MMXManager.prototype.request = function(path, method, data, cb, params){
    ...
    var call = protocol.request({
        ...
        path    : path+((method === 'GET' || params.queryOnly) ? '?'+reqBody : ''),
        method  : method || 'GET',
        ...
        res.on('data', function(chunk){
            ...
            if(!isSuccess(res.statusCode)) winston.error('MMXManager: failed to make remote call to '+ENV_CONFIG.MMX.host+' '+path+': code - ',res.statusCode);
...
// create an mmx app
MMXManager.prototype.createApp = function(userMagnetId, body, cb){
    var me = this;
