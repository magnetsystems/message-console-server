...
function testDB(obj, cb){
    ...
    connection.query('USE '+obj.dbName+';', function(e){
        ...
        winston.info('ORM: successfully connected to existing database "'+obj.dbName+'".');
        cb();
    });
}

function createDB(obj, cb){
    var connection =  mysql.createConnection({
        host     : obj.host,
        por      : obj.port,
        user     : obj.username,
    ...
    connection.query('CREATE DATABASE IF NOT EXISTS '+obj.dbName+';', function(e){
        ...
        winston.info('ORM: successfully created database "'+obj.dbName+'".');
        cb();
    });
}
...
ConfigManager.prototype.setConfig = function(section, configs, cb){
    ...
    if(section == 'Geologging') return require('../lib/Geologger').init(configs, cb);
