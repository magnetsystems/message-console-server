        ...
        MMXManager.provisionServer(obj, {
            ...
            if(code == 200 || code == 201){
                MMXManager.getServerStatus(obj, function(e, status, statusCode){
                    console.log(e, status, typeof status, statusCode);
                    ...
                    me.set('MMX', {
                        ...
                        if(e) return cb(e);
                        winston.info('Config: successfully configured messaging server connectivity to "'+obj.host+'".');
                        cb();
                    });
                }, {
    ...
    me.get(null, function(e, configs){
        ...
        for(var section in configs){
            ...
            for(var item in configs[section]){
                if(allowedConfigs.indexOf(section) != -1 || !isPrivate(item)) cfg[section][item] = configs[section][item];
            }
        }
        cb(null, cfg);
    }, true);
