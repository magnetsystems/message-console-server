...
, configTmplPath = './lib/config/config.json';
...
ConfigManager.prototype.init = function(cb){
    if(ENV_CONFIG.Redis.enabled){
        this.redis = connectRedis(ENV_CONFIG.Redis, cb);
        return;
    }
    cb();
};

ConfigManager.prototype.get = function(feature, cb){
    var me = this;
    if(me.redis && ((Date.now() - me.cachedTime)) > ((ENV_CONFIG.Redis.cacheTimeout || 60) * 1000)){
        me.redis.get('magnetconfig', function(e, res){
            if(e){
                winston.error('Config: error retrieving config from Redis, falling back to retrieval from memory. error message: ', e);
                return cb(null, ENV_CONFIG[feature]);
            }
            winston.verbose('Config: retrieved config feature "'+feature+'" from Redis.');
            ENV_CONFIG = JSON.parse(res);
            me.store(function(e){
                if(e) return cb(e);
                me.cachedTime = Date.now();
                cb(null, ENV_CONFIG[feature]);
            });
        });
    }else{
        cb(null, ENV_CONFIG[feature]);
...
ConfigManager.prototype.setRedis = function(obj, cb){
    ...
    var client = connectRedis(obj, function(e){
        if(e){
        ...
        me.set('Redis', obj, function(e){
            ...
            setConfigsIfNotExist(client, cb);
        });
    });
};
...
function setConfigsIfNotExist(client, cb){
    ...
    function loop(ary, i){
        if(i >= ary.length)
            return cb();
        client.get('magnetconfig:' + ary[i].key, function(e, res){
            if(res == null){
                client.set('magnetconfig:' + ary[i].key, JSON.stringify(ary[i].vals), function(e, res){
                    loop(ary, ++i);
                });
            }else{
                ENV_CONFIG[ary[i].key] = JSON.parse(res);
                loop(ary, ++i);
            }
        });
    }
...
/* CONFIG */
ConfigManager.prototype.getCachedConfigs = function(){
    var cfg = {};
...
ConfigManager.prototype.getCachedConfig = function(section, cb){
    var cfg = {};
