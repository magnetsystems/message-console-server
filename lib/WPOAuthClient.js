...
WPOAuthClient.prototype.getAccessToken = function(user, code, cb) {
    ...
    var me = this;
    me.request('POST', '/oauth/token', 'grant_type=authorization_code&code='+code+'&redirect_uri='+ENV_CONFIG.App.appUrl + '/oauth-login', function(e, oauthSession){
        if(e) return cb(e);
...
WPOAuthClient.prototype.typeMap = function(capability){
    ...
    if(capability.customer || capability.magnetemployee)
        return 'developer';
...
WPOAuthClient.prototype.getUserInfo = function(user, access_token, cb) {
    ...
    me.request('GET', '/oauth/me?access_token='+access_token, null, function(e, data){
        if(e) return cb(e);
        user.id = data.ID;
