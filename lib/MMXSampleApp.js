...
var CONFIGS = {
    ...
    ios     : 'MMXQuickStart/Configurations.plist'
};
...
MMXSampleApp.prototype.getSample = function(userMagnetId, mmxId, platform, cb){
    ...
    MMXManager.getApp(userMagnetId, mmxId, function(e, app){
        ...
        MMXManager.getConfigs(userMagnetId, function(e, mmxconfig){
            if(e) return cb(e);
            if(mmxconfig.configs['mmx.push.callbackurl'] && mmxconfig.configs['mmx.push.callbackurl'] !== '')
                mmxconfig.configs['mmx.push.callbackurl'] = mmxconfig.configs['mmx.push.callbackurl'].trim().replace('http://', '').replace('https://', '');
            var zip = new AdmZip();
