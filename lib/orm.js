...
var singleton = function singleton(){
    ...
    function init(){
        ...
        if(ENV_CONFIG.Database.options.sync){
            sequelize[ENV_CONFIG.Database.options.force === true ? 'drop' : 'sync']().then(
                ENV_CONFIG.Database.options.force === true ? syncDB(onComplete) : onComplete()
            );
        }else{
    ...
    function onComplete(){
        ...
        if(ENV_CONFIG.Database.options.createAdmin){
            require('../lib/startup/Models').setupAdmin(me.callback);
        }else{
            if(typeof me.callback === typeof Function){
                me.callback();
            }
