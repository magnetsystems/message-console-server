...
var singleton = function singleton(){
    ...
    function init(){
        ...
        for(var model in relationships){
            ...
            for(var i=relations.length;i--;){
                winston.verbose("%s %s %s %s", model, relations[i].type, relations[i].model, relations[i].constraints ? 'with constraints: '+JSON.stringify(relations[i].constraints) : '');
    ...
    function onComplete(){
        winston.info("Sequelize schemas created successfully");
        require('../lib/startup/Models').modelSetup(me.callback);
    }
