...
var ContentManagement = function(){
    var me = this;
    this.pageDir = './views';
    Jobs.create('CMSPages', 60, function(opts){
...
// recursively check for pages which do not have an update entry associated with the current instance
...
// update the current instance with the page changes and record an update entry
...
// get a CMSPage record from the database by id
...
// create a CMSEntry database record belonging to a CMSPage for the current instance
...
ContentManagement.prototype.updateSinglePage = function(req, callback){
    var filePath = this.pageDir+(typeof req.body.folder != 'undefined' ? '/'+req.body.folder.replace(stripChars, '') : '')+'/'+req.body.filename.replace(stripChars, '')+'.ejs';
    var me = this;
    me.createPage({
        folder   : req.body.folder,
        filename : req.body.filename,
        content  : qs.escape(req.body.data)
    }, function(e, page){
        if(e){
            callback(e);
        }else{
            callback(null, filePath);
            // me.writePage(req.body.folder, req.body.filename, req.body.data, function(e, filePath){
                //callback(null, filePath);
            // });
        }
    });
...
// create a CMSPage record in the database for tracking updates
