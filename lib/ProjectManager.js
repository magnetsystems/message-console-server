...
ProjectManager.prototype.createProjectFolders = function(userId, projectId, callback){
    ...
    if(userId && projectId){
        ...
        console.error('Project: failed to create user/project folders: invalid userId and/or projectId');
...
ProjectManager.prototype.createFolderIfNotExist = function(path, folder, callback){
    fs.mkdir(path+'/'+folder, function(e){
        if(!e || (e.code === 'EEXIST')){
            ...
            console.error('Project: failed to create folder: ',e);
...
ProjectManager.prototype.createFile = function(filePath, str, callback){
    fs.writeFile(filePath, str, function(e){
        if(e){
            console.error('Project: failed to write file "'+filePath+'": ',e);
    ...
    if(Object.prototype.toString.call(project) == '[object Object]'){
        ...
        orm.model('Project').find({
            ...
            if(project){
                ...
                console.error(err + 'project with id: '+ProjectId+' not found');
            }
        }).error(function(e){
            console.error(err + 'error attempting to find project with id: '+ProjectId);
        });
    }
}
                ...
                WSDLManager.saveWSDL(url, function(e, wsdl){
                    if(e){
                        ...
                        project.addWSDL(wsdl).success(function(){
                            ...
                            console.log("Project: successfully add WSDL: " + url);
                            callback(null, wsdl);
                        }).error(function(e){
                            console.error('Project: "' + project.magnetId + '" failed to add WSDL: ',e);
                            callback('add-wsdl-failed');
                        });
                    }
                });
            }else{
                console.error('Project: "' + project.magnetId + '" attempted to access "' + project.name + '" but was not authorized');
