    ...
    UserManager.read(userMagnetId, false, function(e, user){
        if(user){
            if(body){
                ...
                body.version = '1.0';
                if(body.jdbcPort) body.jdbcPort = forceNumeric(body.jdbcPort, 3306);
                if(body.smtpPort) body.smtpPort = forceNumeric(body.smtpPort, 587);
    ...
    me.read(magnetId, function(e, project){
        if(project){
            if(project.configFileStale === true){
                me.generateAndUpdateState(project, function(e, filePath){
                    if(e){
                        callback(e);
                    }else{
                ...
                fs.readFile(filePath, 'ascii', function(e){
                    if(e){
                        ...
                        winston.verbose('Project: requested cached version of project config file for project "' + project.name + '"');
...
// generate project config file and update attributes
ProjectManager.prototype.generateAndUpdateState = function(project, callback){
    var me = this;
    if(project && project.getWSDLs){
...
// render config file, create folders and file, and return path to file
                ...
                if(e){
                    ...
                    me.createFile(path+'/'+project.artifactId+'.mapp', output, function(e, configFilePath){
                        if(e){
                            ...
                            winston.verbose('Project: created config file: "'+project.artifactId+'.mapp" in "'+path+'"');
...
ProjectManager.prototype.createConfigZip = function(projectPath, filename, configFilePath, apnsCertPath, callback){
    ...
    winston.verbose('Project: created config zip: "'+filename+'.zip"');
    callback(targetPath);
};
...
ProjectManager.prototype.renderConfig = function(project, wsdls, callback){
    var me = this;
    if(typeof project !== typeof {} || !project.name || !project.description){
        ...
        orm.model('User').find({
            ...
            fs.readFile(me.configTmplPath, 'ascii', function(e, tmpl){
                if(e){
                    winston.error('Project: failed to read template file: ',e);
                    callback('error-retrieving-file');
                }else{
                    callback(null, ejs.render(tmpl, {
                        user     : user || {},
                        devexVer : packageJSON.version,
                        date     : new Date(),
                        settings : project,
                        sysDBId  : project.groupId+'_SysDB'+project.id,
                        wsdls    : wsdls
                    }));
                }
            });
                ...
                if(e){
                    ...
                    req.on('end', function(){
                        ...
                        project.updateAttributes({
                            ...
                            winston.verbose('Project: uploaded file: "'+fileName+'", type: "'+fileType+'" in "'+path+'"');
                            callback();
                        }).error(function(e){
                            winston.error('Project: project "' + project.name + '" update failed: ' + e);
                            callback('error-updating-project');
                    ...
                    ws.on('error', function(e){
                        winston.error('Project: error uploading project file: '+e);
...
// add WSDL URL
ProjectManager.prototype.addWebServiceURL = function(projectMagnetId, userId, url, callback){
    var me = this;
    // check for a project matching the specified magnetId
    me.read(projectMagnetId, function(e, project){
        if(project){
            // validate whether the user owns this project, or the user is an admin
            if(project.UserId == userId){
                WebServiceManager.saveWebService(url, function(e, wsdl){
                    if(e){
                        callback(e);
                    }else{
                        project.addWSDL(wsdl).success(function(){
                            me.setToStale(project);
                            winston.verbose("Project: successfully add WSDL: " + url);
                            callback(null, wsdl);
                        }).error(function(e){
...
// remove WSDL
ProjectManager.prototype.removeWebServiceURL = function(magnetId, callback){
    var me = this;
    ...
    me.readWSDL(magnetId, function(e, wsdl){
        if(wsdl){
            ...
            wsdl.destroy().success(function(){
                winston.verbose("Project: successfully deleted WSDL: " + wsdl.url);
                callback(null, wsdl);
            }).error(function(e){
...
ProjectManager.prototype.getWebServices = function(magnetId, callback){
    var me = this;
    me.read(magnetId, function(e, project){
        if(project){
            project.getWSDLs().success(function(wsdls){
...
function forceNumeric(str, rep){
    if(!str || isNumeric(str)) return str;
    str = str.replace(/[^0-9]+/g, '');
    return isNumeric(str) ? str : (rep || 0);
}
