    ...
    if(userObj){
        orm.model('User').find({
            ...
            if(!user){
                ...
                winston.verbose('User: user creation of "' + userObj.email + '" failed: user already exists');
    ...
    orm.model('User').find({
        ...
        if(user){
            me.checkRemainingAdmins(user, userObj, function(e){
                ...
                if(userObj.hasMMXApp === false) user.hasMMXApp = userObj.hasMMXApp = false;
...
UserManager.prototype.bootstrapUser = function(user, cb) {
    var me = this;
    me.create(user, function(e, user){
        if(!e){
            MMXManager.createApp(user.email, user.magnetId, {
                name : 'Quickstart'
            }, function(e1, quickstartApp){
                if(!e1){
                    MMXManager.createAppTopic(user.magnetId, quickstartApp.appId, {
                        body : {
                            topicName : 'all_ios'
                        },
                        headers : {
                    ...
                    MMXManager.sendMessage(user.magnetId, quickstartApp.appId, '', {
                        body : {
                            'receipt'  : true,
                            'metadata' : {
                            'content-type'     : 'text',
