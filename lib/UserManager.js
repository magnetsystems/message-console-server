    ...
    return function(req, res, next){
        ...
        if(req.session.user && types.indexOf(req.session.user.userType) != -1 && req.session.user.activated === true){
            ...
            if(isAPI){
                ...
                res.redirect((ENV_CONFIG.App.view == 'developer' ? '/messaging/console' : '/login')+'?status=locked');
            }
        }else{
            if(isAPI){
                ...
                req.session.entryPoint = req.url;
                res.redirect(ENV_CONFIG.App.view == 'developer' ? '/messaging/console' : '/login');
            }
        }
    }
};
...
//                                }
//                            });
                            TokenManager.create(user, function(e){
                                callback(e ? BecomeDeveloperStatusEnum.FAILED : BecomeDeveloperStatusEnum.SUCCESSFUL, user);
                            });
                        }).error(function(e){
