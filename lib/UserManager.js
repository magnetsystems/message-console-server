...
UserManager.prototype.sendAdminEmail = function(firstName, lastName, email) {
    var subject = 'Messaging Invitation Request';
    ...
    orm.model('User').find({
        ...
        if(!user){
            ...
            orm.model('User').create(userObj, ['firstName', 'lastName', 'email', 'companyName', 'userType', 'inviterId', 'invitedEmail' ]).success(function(user){
                ...
                // Send email to User
                var subject = firstName+' has invited you to Messaging';
                var extra = 's=w';
                EmailService.sendEmail({
                    ...
                    html    : EmailService.renderTemplate({
                        ...
                        vars : {
                            ...
                            resourceUrl  : ENV_CONFIG.App.appUrl+'/resources',
                            url          : ENV_CONFIG.App.appUrl + '/?a=confirm-introduce&' + extra + '&t=' + user.magnetId
                        }
                    }),
                    success : function(){
