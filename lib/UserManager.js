        ...
        where : {
            ...
            if (user) {
                ...
                user.updateAttributes({userType: 'approved'}, [ 'userType' ]).success(function(){
                    ...
                    winston.log('info', 'Approval: user "'+userObj.invitedBy+'" approved user "' + user.id + '"('+user.email+') using "'+(isInvitedByAdmin ? 'admin email invite' : 'admin page approval')+'" successfully at: '+new Date(), {
                        userId      : userObj.invitedBy,
                        targetModel : 'User',
                        targetId    : user.id
                    });
        ...
        where : {
            ...
            if (user) {
                ...
                bcrypt.hash(userObj.password, 10, function(err, hash) {
                    ...
                    winston.log('info', 'EULA: user "'+user.id+'"('+user.email+') accepted the EULA at: '+userObj.dateAcceptedEULA, {
                        userId : user.id
                    });
