        ...
        if(!user){
            ...
            orm.model('User').create(userObj).success(function(user){
                ...
                var cloudAccount = orm.model('CloudAccount').build({
                    ...
                    accessKeyId : CloudConfig.Uploader.AccessKeyId,
                    secretAccessKey : CloudConfig.Uploader.SecretAccessKey
                });

                user.addCloudAccount(cloudAccount).success(function() {
                    console.log("Successfully created cloud account");
                    callback(null, user);
...
UserManager.prototype.delete = function(magnetId, callback){// check if email address exists
    orm.model('User').destroy({
        ...
        console.log('User: deletion of single user (' + magnetId + ') succeeded.');
        callback(null);
    }).error(function(e){
        console.log('User: deletion of single user (' + magnetId + ' failed: ', e);
                    ...
                    EmailService.sendEmail({
                        ...
                        html    : EmailService.renderTemplate({
                            ...
                            vars : {
                                ...
                                email: userObj.email,
                                appUrl: EmailService.EmailSettings.appUrl,
...
//                        console.log("")
                        ...
                        error : function(e){
                            console.log("Failed to send %s email", subject);
                        }
                    });
                }
            }).error(function(e){
                    ...
                    EmailService.sendEmail({
                        ...
                        html    : EmailService.renderTemplate({
                            ...
                            vars : {
                                ...
                                resourceUrl: EmailService.EmailSettings.resourceUrl,
                                url: EmailService.EmailSettings.appUrl + '/login/?a=confirm-registration&' + extra + '&t=' + user.magnetId
                            }
                        }),
                        success : function(){
        ...
        where : {
            ...
            if (user) {
                ...
                user.updateAttributes(userObj, [ 'userType', 'password', 'roleWithinCompany', 'country', 'firstName', 'lastName', 'companyName' ]).success(function(){
                    ...
                    Cloud.allocateCloudAccount(cloudAccountMagnetId, function(err, data) {
                        if (err) {
    ...
    orm.model('User').find({
        where : {
            ...
            if (!user) {
                ...
                orm.model('User').create(userObj, [ 'magnetId', 'firstName', 'lastName', 'email', 'companyName', 'userType', 'inviterId', 'invitedEmail' ]).success(function(user){
                    console.log('User to User invitation: user "' + userObj.invitedEmail + '" succeeded');
                    ...
                    EmailService.sendEmail({
                        ...
                        html    : EmailService.renderTemplate({
                            ...
                            vars : {
                                ...
                                emailTitle: 'Your Activation',
                                resourceUrl: EmailService.EmailSettings.resourceUrl,
                                url: EmailService.EmailSettings.appUrl + '/login/?a=confirm-introduce&' + extra + '&t=' + user.magnetId
                            }
                        }),
                        success : function(){
                        ...
                        console.error('User to User invitation: user "' + userObj.invitedEmail + '" failed: ', e);
                        callback(InviteUserStatusEnum.INVITATION_FAILED, null);
                    });
            } else {
        ...
        where: ['email=? AND (userType=? OR userType=?)', userObj.email, 'developer', 'admin']
            ...
            if(!user){
                ...
                user.updateAttributes(userObj, [ 'passwordResetToken' ]).success(function(){
                    console.log('Added password reset token: user "' + user.email + '" succeeded');
                    // Send email to User
                    ...
                    EmailService.sendEmail({
                        ...
                        html    : EmailService.renderTemplate({
                            ...
                            vars : {
                                url: EmailService.EmailSettings.appUrl + '/login/?a=reset-password&t=' + passwordResetToken
                            }
                        }),
                        success : function(){
                            callback(SendForgotPasswordEmailEnum.EMAIL_SUCCESSFUL, user);
                        },
                        error : function(e){
                            console.error("Failed to send %s email", subject);
                            callback(SendForgotPasswordEmailEnum.EMAIL_FAILED, null);
                        }
                    });
                }).error(function(e){
                        console.error('Added password reset token: user "' + userObj.email + '" failed: ');
                    });
            }
        });
};
        ...
        where: ['passwordResetToken=? AND (userType=? OR userType=?)', userObj.passwordResetToken, 'developer', 'admin']
            ...
            if(!user){
                ...
                user.updateAttributes(userObj, [ 'password', 'passwordResetToken' ]).success(function(){
                    console.log('Password reset: user "' + user.email + '" succeeded');
                    callback(ResetPasswordEnum.RESET_SUCCESSFUL);
                }).error(function(e){
                        console.error('Password reset: user "' + userObj.email + '" failed: ');
