...
UserManager.prototype.read = function(magnetId, notAuth, callback){
    var attributes = notAuth ? ['email'] : ['id', 'magnetId', 'email', 'firstName', 'lastName', 'companyName', 'country', 'roleWithinCompany', 'userType'];
        ...
        if(user){
            bcrypt.hash(password, 10, function(err, hash) {
                ...
                user.save(function(e){
                    if(e){
                        winston.error('Accounts: user "' + sessionUser.username + '" password update failed: ' + e);
