            ...
            if(!user){
                bcrypt.hash(userObj.password, 10, function(err, hash){
                    ...
                    orm.model('User').create(userObj).success(function(user){
                        ...
                        winston.error('User: user creation of "' + userObj.username + '" failed: ', e);
                        callback('create-user-failed');
                    });
                });
            }else{
                winston.error('User: user creation of "' + userObj.username + '" failed: user already exists');
...
UserManager.prototype.registerGuest = function(userObj, isInvitedByAdmin, callback){
    ...
    orm.model('User').find({
        ...
        if(!user){
            userObj.userType = ENV_CONFIG.App.skipAdminApproval === true ? 'approved' : 'guest';
            // Was this user invited?
            if(!userObj.magnetId){
                // Generate the User UUID
                orm.model('User').create(userObj, ['firstName', 'lastName', 'email', 'companyName', 'userType', 'inviterId', 'invitedEmail' ]).success(function(user){
                    ...
                    if(!isInvitedByAdmin){
                        // Send email
                        if(ENV_CONFIG.App.skipAdminApproval === true){
                ...
                orm.model('User').find({
                    ...
                    if(invitedUser){
                        invitedUser.updateAttributes(userObj, [ 'firstName', 'lastName', 'email', 'companyName', 'userType']).success(function(user){
                            ...
                            callback(RegisterGuestStatusEnum.REGISTRATION_SUCCESSFUL, user);
                            // Send email to Magnet Admin
                            if (!isInvitedByAdmin && ENV_CONFIG.App.skipAdminApproval !== true) {
                                // Send email
                                UserManager.prototype.sendAdminEmail(userObj.firstName, userObj.lastName, userObj.email);
                            }else if(ENV_CONFIG.App.skipAdminApproval === true){
                                me.sendCompleteRegistrationEmail(user, isInvitedByAdmin, null, userObj.source);
                            }
                        }).error(function(e){
                            winston.error('Registration: user "' + userObj.email + '" failed: ', e);
            ...
            EmailService.sendEmail({
                ...
                html    : EmailService.renderTemplate({
                    ...
                    vars : {
                        ...
                        resourceUrl : ENV_CONFIG.Email.appUrl+'/resources'
                    }
                }),
                success : function(){
                },
    ...
    EmailService.sendEmail({
        ...
        html    : EmailService.renderTemplate({
            ...
            vars : {
                ...
                resourceUrl : ENV_CONFIG.Email.appUrl+'/resources',
                url         : ENV_CONFIG.Email.appUrl+'/#/complete-register?' + extra + '&t=' + user.magnetId
            }
        }),
        success : function(){
                ...
                EmailService.sendEmail({
                    ...
                    html    : EmailService.renderTemplate({
                        ...
                        vars : {
                            ...
                            resourceUrl  : ENV_CONFIG.Email.appUrl+'/resources',
                            url          : ENV_CONFIG.Email.appUrl + '/?a=confirm-introduce&' + extra + '&t=' + user.magnetId
                        }
                    }),
                    success : function(){
                ...
                EmailService.sendEmail({
                    ...
                    html    : EmailService.renderTemplate({
                        ...
                        vars : {
                            ...
                            resourceUrl  : ENV_CONFIG.Email.appUrl+'/resources',
                            url          : userObj.source ? (userObj.source+'/messaging/console/#/reset-password?t='+passwordResetToken) : (ENV_CONFIG.Email.appUrl+'/resources' + '/account/reset-password?t=' + passwordResetToken)
                        }
                    }),
                    success : function(){
