 ...
 */
UserManager.prototype.registerGuest = function(userObj, callback, isInvitedByAdmin){
    // check if email address exists
    orm.model('User').find({
        where : {
            email : userObj.email
...
UserManager.prototype.approveUser = function(userObj, callback, isInvitedByAdmin){
    // check if magnetId exists
    orm.model('User').find({
        where : {
