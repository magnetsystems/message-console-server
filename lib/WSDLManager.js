...
WSDLManager.prototype.request = function(url, callback){
    ...
    protocol.get({
        ...
        winston.error('WSDLManager: failed to make remote call to '+reqObj.hostname+': ',e);
...
WSDLManager.prototype.parse = function(str, callback){
    ...
    if(str.substr(0, 30).indexOf('<?xml ') != -1){
        parser.parseString(str, function(e, json){
            if(e){
                winston.error('WSDLManager: error parsing WSDL: ',e);
        ...
        winston.error('WSDLManager: error parsing WSDL: not a valid XML file');
...
WSDLManager.prototype.getServiceName = function(json){
    try{
        ...
        //winston.error('WSDLManager: error obtaining service name: ',e);
        return false;
    }
};
...
WSDLManager.prototype.saveWSDL = function(url, callback){
    ...
    if(url && url.trim() != ''){
        ...
        winston.error('WSDLManager: failed to make remote call due to invalid url: '+url);
...
WSDLManager.prototype.create = function(wsdlObj, callback){
    ...
    orm.model('WSDL').create(wsdlObj).success(function(wsdl){
        ...
        winston.error('WSDLManager: error creating WSDL: ',e);
