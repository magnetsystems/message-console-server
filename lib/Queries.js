...
Queries.prototype.findAll = function(req, callback){
    ...
    console.info('Models: retrieving collection with query object: ',queryObj);
    orm.model(modelName).findAll(queryObj).success(function(col){
        callback(col);
    }).error(function(e){
        console.error('Models: error retrieving collection: ',e);
...
Queries.prototype.find = function(req, callback){
    var modelName = req.params.model[0].toUpperCase() + req.params.model.slice(1).slice(0, - 1);
    console.info('Models: retrieving model "'+req.params.model+'" with id "'+req.params.id+'"');
    orm.model(modelName).find({
        ...
        console.error('Models: error retrieving model: ',e);
